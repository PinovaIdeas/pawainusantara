...
------------------------------------
Listens on http://127.0.0.1:8191 (or PORT env var).

POST /solve
  Body (JSON): {"sitekey": "...", "siteurl": "https://example.com", "action": "vote"}
  Response:    {"token": "...", "elapsed": 4.23}
               {"error": "..."} on failure

GET /health -> {"status":"ok","workers":N,"active":a,"queued":q}

Based on EzSolver by Ismoiloff (MIT), with an added `action` field so tokens
match the origin widget (rendered with action:"vote").
"""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        t0 = time.time()
        try:
            print(f"[service] solving sitekey={sitekey!r} action={action!r} "
...
if __name__ == "__main__":
    ...
    print(f"[service] Turnstile solver service running on http://127.0.0.1:{PORT}")
